/* SNES controller for GameCube, Wii, Wii U* and Switch** v2.4.
...
int buttonMode; // 0 = Normal, 1 = GameCube layout, 2 = Game Boy Advance
...
// Hold with X for Game Boy Advance layout
if (digitalRead(buttonX) == LOW && digitalRead(buttonZ) == LOW && digitalRead(buttonL) == LOW && digitalRead(buttonR) == LOW && millis() - timer >= 500) { // 
  buttonMode = 2;
}
...
if (digitalRead(buttonSTART) == LOW && digitalRead(buttonZ) == LOW && digitalRead(buttonL) == LOW && digitalRead(buttonR) == LOW && millis() - timer >= 1500) { // 
  ...
  switch (buttonMode) {
    ...
    case 2: // Game Boy Advance - A = A, B = B, Y = B, X = B
      if (digitalRead(buttonA) == LOW)pinA = 1;
      if (digitalRead(buttonB) == LOW)pinB = 1;
      if (digitalRead(buttonX) == LOW)pinB = 1;
      if (digitalRead(buttonY) == LOW)pinB = 1;
    break;
  }
